...
contract SunAPENFT_TRXPool is LPTokenWrapper, IRewardDistributionRecipient {
    // sunToken
    ...
    function doContinue(bool changed) internal{
        if (block.timestamp > periodFinish && DURATION_NEXT > 0) {
    ...
    function setNextRewardInfo(bool can, uint256 reward, uint256 DURATION_NEW)
    external
   ...
   function notifyRewardAmount(uint256 reward,uint256 new_DURATION)
   external
   onlyRewardDistribution
    ...
    */
    ...
    function migrate(address nextPool) public returns (bool){
        ...
        require(SunAPENFT_TRXPool(nextPool).stakeTo(msg.sender, userBalance), "stakeTo failed");
